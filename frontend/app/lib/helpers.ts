...
export const createPageWithAI = async (prompt: string): Promise<Page> => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE}/block/create`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ prompt }),
  });
  if (!res.ok) {
    ...
    throw new Error(err.error || 'Failed to create page with AI');
  }
  return res.json();
};
